...
import { migrateDbRawSource } from "../migrate-db-raw";
...
describe("migrateDbRawSource", () => {
  it("rewrites a global fetchOne with type args to db.global().fetchOne()", () => {
    ...
    const result = migrateDbRawSource(input, "global-fetch.ts", { globalTables: DEFAULT_GLOBAL_TABLES });
  ...
  it("rewrites a global selectMany with options to db.global().selectMany()", () => {
    ...
    const result = migrateDbRawSource(input, "global-select.ts", { globalTables: DEFAULT_GLOBAL_TABLES });
  ...
  it("rewrites own-tenant filtering via event.user.tenantId", () => {
    ...
    const result = migrateDbRawSource(input, "own-tenant-event.ts", { globalTables: DEFAULT_GLOBAL_TABLES });
  ...
  it("leaves unsafe call sites untouched and reports them as manual", () => {
    ...
    const result = migrateDbRawSource(input, "manual-only.ts", { globalTables: DEFAULT_GLOBAL_TABLES });
    ...
    const insertOneSite = byText.get('insertOne(ctx.db.raw, auditTable, { tenantId, action: event.payload.action })');
  ...
  it("prunes an unused free-function import after rewriting its only call site", () => {
    ...
    expect(result.output).not.toContain('import { fetchOne } from "@cosmicdrift/kumiko-framework/bun-db";');
    expect(result.output).toBe(expected);
  });
  ...
  it("keeps a still-referenced sibling import while pruning the rewritten one", () => {
    ...
    const result = migrateDbRawSource(input, "import-pruning.ts", { globalTables: DEFAULT_GLOBAL_TABLES });

    expect(result.output).toBe(expected);
    expect(result.output).toContain('import { selectMany } from "@cosmicdrift/kumiko-framework/bun-db";');
