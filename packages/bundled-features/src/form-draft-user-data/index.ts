...
export const formDraftUserDataFeature = defineFeature("form-draft-user-data", (r) => {
  r.describe(
    "GDPR (Art. 20 export / Art. 17 erasure) coverage for the `form-draft` feature's `form-draft` entity. Mounts the EXT_USER_DATA export hook so a user's saved drafts are included in the user-data export bundle; the delete hook is a deliberate no-op because `draft` is annotated `userOwned` on the entity — erasure runs via crypto-shredding (destroying the owner's subject key) instead of a physical delete. Kept separate from `form-draft` so form-draft consumers without the user-data-rights pipeline don't pull a hard dependency — requires `user-data-rights`, optionalRequires `form-draft`.",
  );
