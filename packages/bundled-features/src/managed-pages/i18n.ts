// @runtime client
// Pure-Data (i18n-Keys) — client-marked, damit das web/-Bundle sie in den
// Browser-Store pivotieren darf; feature.ts (server) importiert sie ebenso
// (server darf client-safe Pure-Data importieren, wie PAT constants.ts).
//
// Übersetzungs-Bundle für die managed-pages Admin-Screens. Ohne diese Keys
// rendert die UI die Roh-i18n-Keys (screen:*.title, entity:*:field:*, Section-
// Header). Der Boot-Validator (required-surface-keys) verlangt — sobald ein
// Feature r.translations ruft — den KOMPLETTEN Required-Surface-Satz in de+en;
// diese Datei deckt jeden Screen/Feld/Section-Key ab, den managed-pages
// registriert. Die zwei customCss-Keys werden unconditional geliefert (schaden
// bei allowCustomCss:false nicht, sind aber Pflicht für Consumer die es aktivieren).

type LocalizedString = { readonly de: string; readonly en: string; readonly es: string };

export const MANAGED_PAGES_I18N: Readonly<Record<string, LocalizedString>> = {
  // Screen-Titel (global/unpräfixiert — screenTitleKey = screen:<id>.title)
  "screen:page-list.title": { de: "Seiten", en: "Pages", es: "Páginas" },
  "screen:page-edit.title": { de: "Seite bearbeiten", en: "Edit page", es: "Editar página" },
  "screen:branding-settings.title": { de: "Branding", en: "Branding", es: "Marca" },

  // Page-Entity Feld-Labels (managed-pages:entity:page:field:*)
  "managed-pages:entity:page:field:slug": { de: "Slug", en: "Slug", es: "Slug" },
  "managed-pages:entity:page:field:lang": { de: "Sprache", en: "Language", es: "Idioma" },
  "managed-pages:entity:page:field:title": { de: "Titel", en: "Title", es: "Título" },
  "managed-pages:entity:page:field:published": {
    de: "Veröffentlicht",
    en: "Published",
    es: "Publicado",
  },
  "managed-pages:entity:page:field:published:option:true": {
    de: "Veröffentlicht",
    en: "Published",
    es: "Publicado",
  },
  "managed-pages:entity:page:field:published:option:false": {
    de: "Entwurf",
    en: "Draft",
    es: "Borrador",
  },
  "managed-pages:entity:page:field:description": {
    de: "Beschreibung",
    en: "Description",
    es: "Descripción",
  },
  "managed-pages:entity:page:field:ogImage": { de: "OG-Bild", en: "OG image", es: "Imagen OG" },
  "managed-pages:entity:page:field:body": { de: "Inhalt", en: "Content", es: "Contenido" },

  // Config-Edit Feld-Labels (managed-pages:entity:__config-edit__:field:*, camelCase Form-Keys)
  "managed-pages:entity:__config-edit__:field:title": {
    de: "Website-Titel",
    en: "Site title",
    es: "Título del sitio",
  },
  "managed-pages:entity:__config-edit__:field:description": {
    de: "Beschreibung",
    en: "Description",
    es: "Descripción",
  },
  "managed-pages:entity:__config-edit__:field:siteUrl": {
    de: "Website-URL",
    en: "Site URL",
    es: "URL del sitio",
  },
  "managed-pages:entity:__config-edit__:field:accentColor": {
    de: "Akzentfarbe",
    en: "Accent color",
    es: "Color de acento",
  },
  "managed-pages:entity:__config-edit__:field:logoUrl": {
    de: "Logo-URL",
    en: "Logo URL",
    es: "URL del logo",
  },
  "managed-pages:entity:__config-edit__:field:layoutPreset": {
    de: "Layout",
    en: "Layout",
    es: "Diseño",
  },

  // Section-Header
  "managed-pages:section.meta": { de: "Metadaten", en: "Metadata", es: "Metadatos" },
  "managed-pages:section.body": { de: "Inhalt", en: "Content", es: "Contenido" },
  "managed-pages:branding.section.identity": { de: "Identität", en: "Identity", es: "Identidad" },

  // Row-Actions + Confirm (pageListScreen)
  "managed-pages:actions.edit": { de: "Bearbeiten", en: "Edit", es: "Editar" },
  "managed-pages:actions.delete": { de: "Löschen", en: "Delete", es: "Eliminar" },
  "managed-pages:confirms.page-delete": {
    de: "Diese Seite wirklich löschen?",
    en: "Delete this page?",
    es: "¿Seguro que quieres eliminar esta página?",
  },

  // Nur bei allowCustomCss:true im Screen — unconditional geliefert
  "managed-pages:entity:__config-edit__:field:customCss": {
    de: "Eigenes CSS",
    en: "Custom CSS",
    es: "CSS personalizado",
  },
  "managed-pages:branding.section.custom-css": {
    de: "Eigenes CSS",
    en: "Custom CSS",
    es: "CSS personalizado",
  },
};
