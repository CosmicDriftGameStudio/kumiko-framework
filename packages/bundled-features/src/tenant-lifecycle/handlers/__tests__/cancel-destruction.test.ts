...
// cancel-destruction.write.ts's Temporal.Instant.compare call — the write
...
// unrelated ctx.tz setup; see tenant-lifecycle.integration.test.ts for the
// end-to-end coverage of this handler with Temporal intact).

import { describe, expect, test } from "bun:test";
import { Temporal as TemporalPolyfill } from "temporal-polyfill";
import { isWithinGracePeriod } from "../cancel-destruction.write";
...
describe("isWithinGracePeriod — kumiko-framework#1525: no ambient Temporal global", () => {
  ...
  test("future vs. past gracePeriodEnd, without relying on globalThis.Temporal", () => {
    ...
    // isWithinGracePeriod's own cast — see cancel-destruction.write.ts.
