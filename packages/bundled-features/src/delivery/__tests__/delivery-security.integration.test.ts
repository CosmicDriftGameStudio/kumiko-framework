...
import { createTestUser, setupTestStack, type TestStack, unsafePushTables } from "@cosmicdrift/kumiko-framework/stack";
...
describe("delivery access matrix", () => {
  test("delivery log handler and screen share access.admin", () => {
    ...
    expect(rolesOf(registry.getQueryHandler(DeliveryQueries.log)?.access)).toEqual([...access.admin]);
...
describe("delivery log HTTP access", () => {
  test("TenantAdmin can query delivery log", async () => {
    ...
    const res = await stack.http.queryOk<{ rows: readonly unknown[] }>(DeliveryQueries.log, {}, user);
