...
// head-less document (nothing to splice into). `tags` is pre-rendered HTML
// from a caller (renderApexHeadTags) — this function only owns placement +
// the original <title> removal, not escaping.
export function injectPageHead(html: string, tags: string): string {
