...
- Updated dependencies [5d3b3e8]
  ...
  - @cosmicdrift/kumiko-http@0.318.0

## 0.317.0

### Minor Changes

- b13c820: files: a valid upload (png/jpeg/gif/webp/pdf/doc/docx) under the wrong extension is normalized to its real type instead of rejected, when that type is in the field's accept list or the upload has no accept restriction at all
...
- 94ce380: runInOwnTransaction(tenantDb, fn) runs fn in a fresh transaction on a TenantDb's pool runner
...
- fd40653: `parseChangesetChanges` no longer breaks on a blank line inside a `key: |` block
  ...
  - @cosmicdrift/kumiko-http@0.317.0
...
- aa32979: kumiko-schema generate rejects unrecognized trailing arguments instead of silently ignoring them
...
- Updated dependencies [aa32979]
  - @cosmicdrift/kumiko-types@0.316.0
  - @cosmicdrift/kumiko-http@0.316.0

## 0.315.0

### Minor Changes

- 0f2643d: files: content verification is now keyed by filename extension, covering every signature-bearing type
...
- @cosmicdrift/kumiko-http@0.315.0
...
- @cosmicdrift/kumiko-http@0.311.0
...
- Updated dependencies [4f36c3f]
  - @cosmicdrift/kumiko-types@0.310.0
  - @cosmicdrift/kumiko-http@0.310.0
...
- @cosmicdrift/kumiko-http@0.303.0
- @cosmicdrift/kumiko-types@0.303.0

## 0.302.0

### Minor Changes

- deede20: Anonymous write handlers whose input accepts a personal-data field must declare access.personalData: "public-intake" (fw#2885)
...
- @cosmicdrift/kumiko-http@0.302.0
- @cosmicdrift/kumiko-types@0.302.0

## 0.301.0

### Minor Changes

- bae6958: Add shared /metrics wiring: `prometheusMetricsEnvSchema` and `resolveObservabilityWiring` under `@cosmicdrift/kumiko-framework/observability`
...
- @cosmicdrift/kumiko-http@0.301.0
- @cosmicdrift/kumiko-types@0.301.0

## 0.300.0

### Minor Changes

- 2414932: Remove the guest-identity all-role: unauthenticated handlers must declare roles: ["anonymous"] with a rateLimit
...
- @cosmicdrift/kumiko-http@0.300.0
- @cosmicdrift/kumiko-types@0.300.0
...
- 4ae8163: Anonymous extraRoutes get `write`, running as the session the /api chain resolved for this request (never more than that caller could do via /api/write, request-resolved tenant, /api/ only).
...
- 6735981: extraRoutes/hostDispatch move to structured route and wire definitions
...
- @cosmicdrift/kumiko-http@0.298.0
...
- cb6e8a4: entityEdit accepts `layout.mode: "tabs"` (fw#3134)
...
- bfa7536: refEntity on projectionList/relatedList columns and projectionDetail fields is boot-checked against registered entities
...
- 42b0562: kumiko schema apply can resolve the schema-declared Key Manager slots
...
- 7f6fbc6: Fix `parseEnv` throwing for a refined schema with a ciphertext-only Key Manager slot
...
- ea10c90: Declare Key Manager ciphertext slots in the env schema
...
- 7fb8a62: Resolve the blind-index key from a Key Manager ciphertext
...
- 78f9c42: appendProvenanceEvent enforces event.tenantId === db.tenantId
...
- efac5bb: Boot fails when an anonymous-accessible handler declares no rateLimit
...
- 20853fa: ctx.db.unsafeRaw is denied under ctx.queryAsMember
...
- 1e5a8e0: Drawer gains a modal prop for non-modal side panels
...
- f01015e: Entity graph traversal helpers for cross-tenant ownership changes
...
- dea8ea0: Event-store executor writes accept a declarative `expect:` precondition
...
- 2e868a7: PLATFORM_KEK can now be resolved from a Key Manager ciphertext at boot
...
- 253ade3: requiredKeysFromScreen reads section.groups — group field labels and group titles are i18n-required
...
- a84d3cb: EventDispatcher gains drain() for race-free test teardown
...
- 8ee38b3: Add a `tooling` repo kind, scoped out of guards by default
  ...
  - @cosmicdrift/kumiko-http@0.285.0
  - @cosmicdrift/kumiko-types@0.285.0

## 0.284.0

### Patch Changes

- Updated dependencies [4880f4e]
...
- @cosmicdrift/kumiko-types@0.282.0
...
- fae19a6: TenantDataHookCtx/UserDataHookCtx.db is a tenant-filtered TenantDb; unfiltered hook access needs r.useExtension({ escapeHatch }) (fw#2914 Zugang 3).
...
- @cosmicdrift/kumiko-types@0.280.0
...
- Updated dependencies [01c9133]
  - @cosmicdrift/kumiko-types@0.273.0
...
- Updated dependencies [d0184f7]
  - @cosmicdrift/kumiko-types@0.264.0
...
- cd255ca: Fix: field `unit` and entity `defaultCurrency` were dropped from the client schema. `buildAppSchema` now forwards a number field's `unit` (static string or `{ field }`) and the entity's `defaultCurrency`, so the edit-form unit suffix renders and money fields use the declared currency instead of always falling back to "EUR". The same projection gap is closed for other properties the edit view-model reads: text `format` (password masking), timestamp `locatedBy` (wall-clock input), file/image `accept`/`maxSize`/`variants`, and decimal `scale` (incl. embedded sub-fields).
...
- 9b74bc9: fw#2809: **BREAKING** — `encryptPiiFieldValues` and `resolveSubjectForField` now require `entityName` on their options argument (`EncryptPiiOptions.entityName` / `ResolveSubjectOptions.entityName`), and `encryptForDirectWrite` takes a new `entityName` parameter.
  ...
  ```ts
  await encryptPiiFieldValues(row, entity, piiFields, kms, kmsCtx, {
...
- @cosmicdrift/kumiko-types@0.252.1
...
- Updated dependencies [109ff0d]
  - @cosmicdrift/kumiko-types@0.248.0
...
- f2c9178: Fix cross-tenant data access in the entity `restore` verb. `restore()` loaded its
...
- @cosmicdrift/kumiko-types@0.243.3
...
- Updated dependencies [efd5891]
  - @cosmicdrift/kumiko-types@0.242.0
...
- Updated dependencies [2fbab3d]
  - @cosmicdrift/kumiko-types@0.239.0
...
- Updated dependencies [8206493]
  - @cosmicdrift/kumiko-types@0.238.0
...
- Updated dependencies [562b11f]
  - @cosmicdrift/kumiko-types@0.237.2
...
- 1dca3bc: `runSchemaCli` now exits 1 and prints an "Unbekanntes Subcommand" error to
...
- 825acb8: Fix `runProdApp` silently bypassing the Redis-backed cross-replica `SseBroker` default that fw#2630 introduced. `runProdApp` built its own in-memory broker unconditionally and passed it in as `ServerOptions.sseBroker`, which always wins over `buildServer`'s own `REDIS_URL`-gated default — so under `replicas > 1` in production, SSE broadcast and access-invalidation events never reached any pod other than the one that happened to own the shared event-store cursor.
  ...
  - @cosmicdrift/kumiko-types@0.236.1
...
- 69529cd: `list()` now checks field-level read access before filtering or sorting by a field, closing an oracle that let a caller probe a stripped field's values through result counts or row order.
...
- c5a5dd4: Event-dispatcher pass failures no longer vanish silently. A thrown error
...
- 41ef079: `kumiko-upgrade` filters pending changes against the recorded upgrade marker instead of the installed version.
  - @cosmicdrift/kumiko-types@0.235.4

## 0.235.3

### Patch Changes

- fa9a541: `runPendingRebuilds` no longer logs a data-loss error for a managed table that a later migration in the same run deliberately DROPped after migrating its data elsewhere. Previously such a table was reported as `unresolvedManaged` ("now EMPTY and were NOT rebuilt ... Restore the owning feature") even though it no longer exists — it now drains silently as `unmapped` instead.
  - @cosmicdrift/kumiko-types@0.235.3
...
- 5882fb3: Documents the breaking `UnprocessableOpts.details` tightening from #2460 (never released with a changeset) and ships a codemod for it.
...
- aeac06f: Fix `r.screen({ nav: {...} })` writing `icon`/`parent`/`order` as explicit `undefined` when a screen omits those optional nav fields, which made `buildAppSchema` warn "Output ist nicht JSON-safe" on every boot outside production even though `JSON.stringify` already dropped the key and no consumer was affected.
- 03a8df0: `waitFor` now polls when a callback returns `false` (a boolean-predicate style), not only when it throws — a non-throwing predicate previously returned after a single call without ever waiting.
- Updated dependencies [05bdf93]
  - @cosmicdrift/kumiko-types@0.232.0
...
- @cosmicdrift/kumiko-types@0.229.1

## 0.229.0

### Minor Changes

- ebadd51: `perTenant` jobs no longer require an app to supply `getActiveTenantIds` when the tenant feature is mounted; the runner resolves tenants itself via the tenant feature's `tenant:query:active-tenant-ids` query, and `server-runtime`/`dev-server` now thread an explicit `getActiveTenantIds` through when an app does provide one. Per-tenant fan-out children now get a deterministic job id derived from the wrapper run, so a perTenant wrapper that fires twice for the same trigger (a retry, a Redis drop, an instance restarting mid-run) no longer enqueues duplicate children per tenant.
...
- 211ff70: Fix `migrate generate` silently swallowing an index predicate/column/uniqueness change into the new snapshot without emitting DDL. A same-name index whose `columns`, `unique`, `whereSql`, or `needsManualWhere` changed between snapshots is now detected as a table diff and rendered as `DROP INDEX` + `CREATE INDEX` (or left safely commented out when the new predicate needs manual review). Also fixes a related bug where a brand-new partial index with an unrenderable WHERE clause was emitted as a live, uncommented `CREATE INDEX` with the predicate silently dropped. Consumer apps upgrading past this should run `kumiko-schema generate <name>` and commit the resulting migration: an app whose snapshot predates the `is_deleted` predicate change can carry pre-existing, previously-invisible drift that now surfaces as a failing schema-drift check. `read_users_email_unique_bidx` is the case to watch — if its snapshot predates that predicate change, it's missing `AND "is_deleted" = false` in `whereSql`, and this is product-behavior-relevant, not just DDL cleanup: without the regenerated migration, a soft-deleted user's email stays blocked from re-registration. The framework's own reference apps already carry this migration (`demo/kumiko/migrations/0005_fix-users-bidx-soft-delete-predicate.sql`, `samples/apps/use-all-bundled/kumiko/migrations/0018_fix-users-bidx-soft-delete-predicate.sql`); consumer apps need to generate their own, since migrations live in the app repo, not the framework package.
...
- 415ff22: Fix `projectField()` in the client-schema projection to forward `display`, `columns`, and `maxRows` for `multiSelect` fields.
...
- @cosmicdrift/kumiko-types@0.224.1
...
- Updated dependencies [c4d07f5]
  - @cosmicdrift/kumiko-types@0.223.0
...
- f38784b: fw#2223: the tenant `/members` screen is now a single `projectionList` (`tenant:query:team:list`) merging active memberships and pending invitations into one sortable, searchable, status-faceted list, instead of a custom two-card component. Cancelling a pending invitation is a danger-styled row action visible only on pending rows.
...
- 75a8fd7: List reads are now capped: `eventStore.list()` (and the shared executor list path) validate pagination through a new `resolveListPagination()`. A missing `limit` still falls back to the existing 50-item default, but a non-integer or negative `limit` or `offset` now throws `UnprocessableError` (`invalid_list_limit` / `invalid_list_offset`) instead of reaching SQL, and any `limit` above `MAX_LIST_LIMIT` (200) is clamped. Previously a caller-supplied `?limit=` string was interpolated into the `LIMIT` clause unvalidated, which both allowed oversized result sets and let a crafted numeric-looking value inject SQL. Apps that intentionally page beyond 200 rows must loop with `offset` instead.

  Schema-definition boundaries now reject unbranded `{ kind: "sql-expr", text }` objects outright: `dialect.sql()` and `column.default()` no longer treat a duck-typed object that merely _looks_ like a `SqlExpression` as one — it is quoted as a JSON value instead of spliced into DDL/query text. Only expressions created through the framework's `sql()`/branded helpers carry the brand check now enforced in `literalDefault()`, `dialect.sql()`, and `sqlExpressionText()` (entity-table-meta). A request-supplied or persisted payload that mimics the shape can no longer fake a raw-SQL literal.
...
- 52b0ba6: Several defensive-hardening fixes:
...
- 3d1a0dd: Write handlers now get `ctx.dbOutsideTransaction`, a tenant-scoped handle on the unbound connection pool that isn't tied to the handler's own transaction. Writes made through it survive a rollback of the handler's tx — use it to record facts about side effects that already happened outside the database (e.g. a paid external call) before the handler throws.
- 18c7fc1: Adds `UploadZone` to the widget catalog (`@cosmicdrift/kumiko-renderer-web`) — a drop-zone/multi-file-picker widget with per-file upload status, for apps that need a proper upload surface without reaching for raw HTML tags (fw#1831). `ButtonProps` also gains optional `className`/`ref` so a `Button` can be composed into custom drop targets.
...
- 9344050: `listSchema` in `engine/entity-handlers.ts` wasn't exported, so consumers had to copy the entity-list handlers' query-schema shape into their own code. Those copies silently drifted from the framework contract whenever a field was added. Fix: exported as `entityListSchema` from `@cosmicdrift/kumiko-framework/engine` so consumers can import instead of duplicating.
...
- 85102ca: Content-Collections: `ownership: "user"` implementiert
...
- 9400ec1: template-resolver: Feature-Beschreibung deckt jetzt Content-Collections und kind `text-block` ab
...
- 52753b6: Content-Foundation Phase 2: Content-Collections mit eigenen Rollen pro Sammlung
...
- 5dc5290: schema-cli: fix `migration-content drift` false positives on multi-column `ALTER TABLE ... ADD COLUMN a, ADD COLUMN b` statements. The build-time replay used to match only the first `ADD COLUMN`/`DROP COLUMN` clause per statement — a migration adding several columns in one `ALTER TABLE` (a common hand-edit pattern, e.g. `0007_fix-secrets-table-columns`'s three-column fix) reported the later columns as missing even though the migration creates them. `applyStatement` now walks every comma-separated clause in the statement, in order.
