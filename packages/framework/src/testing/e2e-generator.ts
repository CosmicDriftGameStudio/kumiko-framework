// E2E-Generator — leitet Playwright-Specs aus der Registry ab.
//
// Grundidee: jede r.screen(...) ist eine testbare Oberfläche. Der Generator
// iteriert getAllScreens(), baut pro Screen einen Satz TestSpecs (vier Kinds,
// einer pro "was muss mindestens funktionieren") und gibt das Playwright-
// Source-File als String zurück.
//
// Drei Stufen, jede einzeln testbar:
//   1. generateE2ESpec(registry)         — Registry → TestSpec[]
//   2. generateZodFixture(schema)        — ZodType   → plausibler Value
//   3. renderPlaywrightSpec(specs)       — TestSpec[] → Playwright-Source
//
...
// "not supported yet" — wir füllen nach, wenn ein echtes Sample es braucht.
// Fake-komplexe-Werte würden nur False-Green-Tests produzieren.
//
// UI-Vertrag (muss mit dem Renderer in app/ übereinstimmen):
//   - Input pro Feld:        data-testid="field-<name>"
//   - Validation-Hint:       data-testid="field-error-<name>"
//   - Submit-Button-Label:   matches /save|speichern/i (aria-label oder text)
//   - List-Tabelle:          role="table"
// Ändert einer davon, bricht jeder generierte Test. Eine Anpassung gehört
...
// --- Stufe 3: Playwright-Source rendern ---

export function renderPlaywrightSpec(specs: readonly E2ETestSpec[]): string {
  const body = specs.map(renderTest).join("\n\n");
  return `${HEADER}\n\n${body}\n`;
}

const HEADER = `// Auto-generated by @kumiko/framework/testing — do not edit by hand.
...
function renderTest(spec: E2ETestSpec): string {
  switch (spec.kind) {
    case "list-renders":
      return renderListRenders(spec);
    case "list-has-fixture-row":
      return renderListHasFixtureRow(spec);
    case "edit-validates-required":
      return renderEditValidatesRequired(spec);
    case "edit-save-persists":
      return renderEditSavePersists(spec);
  }
}

function renderListRenders(spec: Extract<E2ETestSpec, { kind: "list-renders" }>): string {
  return `test("${spec.title} — list renders", async ({ page }) => {
  await page.goto("${spec.urlPath}");
  await expect(page.getByRole("table")).toBeVisible();
});`;
}

function renderListHasFixtureRow(
  spec: Extract<E2ETestSpec, { kind: "list-has-fixture-row" }>,
): string {
  return `test("${spec.title} — list shows seeded row", async ({ page, request }) => {
  await postWrite(request, "${spec.writeHandlerQn}", ${JSON.stringify(spec.fixture)});
  await page.goto("${spec.urlPath}");
  await expect(page.getByText(${JSON.stringify(spec.identifyingValue)})).toBeVisible();
});`;
}

function renderEditValidatesRequired(
  spec: Extract<E2ETestSpec, { kind: "edit-validates-required" }>,
): string {
  const fields = spec.requiredFields.map((f) => JSON.stringify(f)).join(", ");
  return `test("${spec.title} — edit validates required (${spec.requiredFields.join(", ")})", async ({ page }) => {
  await page.goto("${spec.urlPath}");
...
function renderEditSavePersists(
  spec: Extract<E2ETestSpec, { kind: "edit-save-persists" }>,
): string {
  const fills = spec.fills.map(renderFillOp).join("\n");
  const verify = spec.listUrlPath
    ? `  await page.goto("${spec.listUrlPath}");
  await expect(page.getByText(${JSON.stringify(spec.identifyingValue)})).toBeVisible();`
    : `  await expect(page.getByText(${JSON.stringify(spec.identifyingValue)})).toBeVisible();`;
  return `test("${spec.title} — edit saves and persists", async ({ page }) => {
  await page.goto("${spec.urlPath}");
${fills}
  await page.getByRole("button", { name: /save|speichern/i }).click();
${verify}
...
function renderFillOp(op: EditFillOp): string {
  switch (op.kind) {
    case "check":
      return `  await page.getByTestId("field-${op.field}").setChecked(${op.value});`;
    case "select":
