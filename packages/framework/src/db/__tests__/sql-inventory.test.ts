...
import { formatReport, isRawSqlAllowed, scanRepo } from "../sql-inventory";
...
async function tempRepo(files: Record<string, string>): Promise<string> {
  const root = join(import.meta.dir, `.tmp-sql-inv-${crypto.randomUUID()}`);
  cleanups.push(root);
  await Promise.all(
    Object.entries(files).map(([rel, content]) => Bun.write(join(root, rel), content)),
  );
  return root;
}
...
describe("sql-inventory", () => {
  test("isRawSqlAllowed permits db/queries and bun-db/query", () => {
    expect(
      isRawSqlAllowed(
        "/repo/packages/framework/src/db/queries/event-store.ts",
      ),
    ).toBe(true);
    expect(isRawSqlAllowed("/repo/packages/framework/src/bun-db/query.ts")).toBe(true);
    expect(isRawSqlAllowed("/repo/packages/bundled-features/src/sessions/db/queries/cleanup.ts")).toBe(
      true,
    );
    ...
    expect(isRawSqlAllowed("/repo/packages/bundled-features/src/sessions/handlers/cleanup.job.ts")).toBe(
      false,
    );
  });
