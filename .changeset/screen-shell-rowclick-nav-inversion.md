...
- `PageSection` (primitives/layout.tsx) gains an optional `maxWidth` prop reusing the same 3xl/4xl/full class map as `FormScreenShell` (now defined once and shared); `dashboard-body.tsx`'s hand-rolled list container now renders through `PageSection` instead of duplicating its padding.
