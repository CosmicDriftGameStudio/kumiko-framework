...
**Migration:** Replace every `openToAll: true` with `openToAll: { reason: "<who may call it and why that is safe without a role>" }`, adding `personalData: "tenant-members"` where the boot validator requires it for a write handler accepting unbound personal data. Pattern-library or Designer-authored JSON that still has `openToAll: true` must be re-saved with a reason before it type-checks or passes boot. `bun scripts/migrate-open-to-all.ts --dry-run <paths>` rewrites test files automatically (`--test-reason "<text>"` to override the default reason) and reports every non-test site for a manual reason; drop `--dry-run` to apply the test-file rewrites.
